  ...
  //   echo json_encode($this->data); exit;
    ...
    // fetch program dan kegiatan terlebih dahulu
      if (!$this->ion_auth->logged_in()){
        ...
        $mprgrm = $this->User_model->addprgrm($this->tahunskr);
        $mkegiatan = $this->User_model->addmkegiatan($this->tahunskr);
        $dpa221 = $this->User_model->sinkrondpa221($this->tahunskr,$unit);
        $data['data'][] = array(
          'status'   =>  $dpa221
        );
        echo json_encode($data);
    ...
    function jsondpa(){
      ...
      if (!$this->ion_auth->logged_in()){
       ...
       $modellv1=$this->User_model->lv1($unitkey,$idkeg,$this->tahunskr);
       $matangr1=$this->User_model->mtglv($modellv1['rek'],$this->tahunskr);
       $modellv2=$this->User_model->lv2($unitkey,$idkeg,$this->tahunskr);

       foreach ($modellv2 as $key) {
         $kdrek= $key['rek'];
         $matangr2=$this->User_model->mtglv($kdrek,$this->tahunskr);
         $modellv3=$this->User_model->lv3($unitkey,$idkeg,$this->tahunskr,$kdrek);
         $lv3=array();
         foreach ($modellv3 as $xkey) {
           $matangr3=$this->User_model->mtglv($xkey['rek'],$this->tahunskr);

           $modeldpa22 =$this->User_model->jsondpa22($unitkey,$idkeg,$this->tahunskr,$xkey['rek']);
           $dpa = array();
            ...
            if(!$modeldpa22 ){
              ...
              foreach ($modeldpa22 as $ykey) {
                 $dpa221 = array();

                 $id = $ykey['id'];
                 $unitkey = $ykey['unitkey'];
                 $kdkegunit = $ykey['kdkegunit'];
                 $mtgkey = $ykey['mtgkey'];
                 $nilai = $ykey['nilai'];
                 $tahun = $ykey['tahun'];
                 $kdper = $ykey['kdper'];
                ...
                foreach ($modeldpa221 as $xykey ) {
                  $uraian = $xykey['uraian'];
                  $jumbyek = $xykey['jumbyek'];
                  $satuan = $xykey['satuan'];
                  $tarif = $xykey['tarif'];
                  $jumhar = $jumbyek * $tarif;
                  $deturaianx   = str_replace("-","",$uraian);
                  $fixuraian    = str_replace("  ","",$deturaianx);
                  $dpa221[] = array(
                      'ur'   => $fixuraian,
                      'vl'   => number_format($jumbyek,2),
                      'st'   => $satuan,
                      'hs'   => $this->template->nominal($tarif),
                      'jh'   => $this->template->nominal($jumhar),
                      'tp'   => $type
                  );
                }

                $dpa[] = array(
                    'kr'   => $kdper, //kode_rekening
                    'nr'   => $nmper, //nama rekening
                    'nl'   => $this->template->nominal($nilai), //nilai
           ...
           $lv3[] = array(
               'rek'   => $matangr3['kdper'], //kode_rekening
               'nmrek' => $matangr3['nmper'], //kode_rekening
               'jml' => $this->template->nominal($xkey['jml']), //jumlah
               'det'   => $dpa, //nama rekening
           );
         }
         $lv2[] = array(
             'rek'   => $matangr2['kdper'], //kode_rekening
             'nmrek' => $matangr2['nmper'], //kode_rekening
             'jml' => $this->template->nominal($key['jml']), //jumlah
             'lv3'   => $lv3, //nama rekening
         );
       }

       $lv1[] = array(
           'rek'   => $matangr1['kdper'], //kode_rekening
           'nmrek' => $matangr1['nmper'], //kode_rekening
           'jml' =>  $this->template->nominal($modellv1['jml']), //jumlah
           'lv2'   => $lv2, //nama rekening
    ...
    function struktur(){
        if (!$this->ion_auth->is_kasubag()){
            ...
            // $this->template->load('viewlama/template','v_struktur',$this->data);
             $this->template->load('templatenew','v_struktur',$this->data);
        }
    }
...
//list legiatan di dasbord
    ...
    function entrikegiatan(){
        if (!$this->ion_auth->logged_in()){
            ...
            $listkegiatan=$this->User_model->listkegiatan($idunit,$this->tahunskr);
            if($listkegiatan){
               $this->data= array(
                'idopd'     => $idopd,
                'nmopd'     => $namaopd,
    ...
    if (!$this->ion_auth->logged_in()){
       ...
       /*Dari function ini akan di cek ke tabel struktur apakah user tersebut KADIS/PPK/PPTK */
        ...
        if($struktur ){
            ...
            $peran=$struktur->peran;
            if($peran=='3'){
              $lskeg = $this->User_model->getlistkegiatan($nip,$this->tahunskr);
                  // $this->data= array(
                  //     'idopd'     => $idopd,
                  //     'nmopd'     =>  $namaopd,
                  //     'tahun'     =>  $this->tahunskr,
                  ...
                  foreach ($lskeg as $key => $value) {
                    $kdkeg= $value['kdkegunit'];
                    //sum ke angkas get nilai sum
                    $getnilai =$this->User_model->sumangkas($kdkeg,$idopd,$this->tahunskr);
                    $list[] = array(
                      'id'        => $value['id'],
                      'kdkegunit' => $value['kdkegunit'],
                      'nmkegunit' => $value['nmkegunit'],
                      'nilai'     => $getnilai['nilai'],
                      'idpnspptk' => $value['idpnspptk'],
                      'idpnsppk'  => $value['idpnsppk']
    ...
    //     echo 'hoho';
        ...
        if($struktur ){
            ...
            if($peran=='3'){
                ...
                $secretHash = "yqB6gHWfslCoYKWuAcOkqWbBRGqXqMA0UdNxQyqG9Iw=";
                if(!isset($_GET['unit'], $_GET['keg'], $_GET['tab'],$_GET['pr'],$_GET['indexbl'],$_GET['bl'],$_GET['ub'])) {
                  redirect('User/dafkeg', 'refresh');
                    ...
                    if($unit!=$idopd){
                        ...
                        $arraybuln = array('Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember' );
                        $lskeg = $this->User_model->getdetlistkegiatan_detpptk($nip,$kegiatan,$this->tahunskr);
                        ...
                        $lskeg = $this->User_model->getdetlistkegiatan_detpptk($nip,$kegiatan,$this->tahunskr);
                         if($lskeg){
                            ...
                            $arr = array('jan','feb','mar','apr','mei','jun','jul','ags','sep','okt','nov','des' );
                                ...
                                if($pertama==1){
                                    $header=$this->User_model->getheader_realisasipptk($idopd,$lskeg->kdkeg,$this->tahunskr);
                                    $anggaranopd = $this->User_model->anggaranopd($idopd);
                                    $bobotkeg=$lskeg->nl/$anggaranopd->anggranopd*100;
                                    $this->data= array(
                                        'idopd'     => $idopd,
...
function jsondetkegppk(){
    ...
    $unitkey=$this->input->post('unitkey');
         ...
         // $unitkey='80_';
    $nip=$this->ion_auth->user()->row()->username;
    $lskeg = $this->User_model->getdetlistkegiatan_detppk($nip,$idkeg,$this->tahunskr);
    $data['header'][] = array(
       'kdkegunit'   =>   $lskeg->kdkegunit,
       'nmkegunit'   =>   $lskeg->nmkegunit,
       'nilai'       =>   $lskeg->nilai,
     ...
     if (!$this->ion_auth->logged_in()){
        ...
        if($struktur ){
            ...
            if($peran=='2' || $peran=='6' ){
                ...
                $blnjamodal = $this->User_model->cekblnjamodal($unitkey,$idkeg,$this->tahunskr);
...
// list belanja modal yang akan di entri target
function jsonlistbmodal($unit,$kegiatan){
    header('Content-Type: application/json');
    echo $this->User_model->jsonlistbmodal($unit,$kegiatan,$this->tahunskr);
}

function jsonlistbmodal_spesial($unit,$kegiatan){
    header('Content-Type: application/json');
    echo $this->User_model->jsonlistbmodal_spesial($unit,$kegiatan,$this->tahunskr);
}

function jsonlisturaian_spesial($unit,$kegiatan){
    header('Content-Type: application/json');
    echo $this->User_model->jsonlisturaian_spesial($unit,$kegiatan,$this->tahunskr);
}
//batas list belanja modal yang akan di entri target
...
function tambahmodal(){
  ...
  if (!$this->ion_auth->logged_in()){
    ...
    $idopd =$getopd->unitkey;
    $namaopd=$getopd->nmunit;
    $stat_bmodal=$getopd->stat_kak_bmodal;
    if($struktur ){
      $peran=$struktur->peran;
      $nip=$struktur->nip;
      ...
      if($peran=='2' || $peran='6'){
          if($stat_bmodal != 1){
            ...
            if($statmodal->num_rows() > 0){
              ...
              $insert= $this->User_model->simpanmodalspesial($data);
              if($insert){
                $status = true;
                $ada    = 0;
              }else{
...
function entri_target_belanja_modal(){
    if (!$this->ion_auth->logged_in()){
        redirect('Home/login', 'refresh');
    }elseif ($this->ion_auth->is_admin()){
        redirect('Cpanel', 'refresh');
  ...
  if (!$this->ion_auth->logged_in()){
    ...
    if($struktur ){
      ...
      if($peran=='2' || $peran='6'){
        ...
        $poststat       = $this->input->post('stat');
        $dataupdate=array(
                    'permasalahan'    => $postmslh,
                    'stat_teruskan'   => $poststat,
                    'admin_teruskan'  => $nip
                );
  ...
  if (!$this->ion_auth->logged_in()){
     ...
     /*Dari function ini akan di cek ke tabel struktur apakah user tersebut KADIS/PPK/PPTK */
      ...
      if($struktur ){
          ...
          if($peran=='6' || $peran=='1' ){
              /*jika peran 6 maka Sekretaris dan 1 Kadis */
              ...
              if($modelpptkmaster->num_rows() == 0){
                  ...
                  $datapagu=$this->User_model->paguopd( $this->tahunskr,$this->bulanskr,$idopd);
                  $idtabmasterpptk=$modelpptkmaster->row_array()['id'];
                  $modellstppk= $this->User_model->get_list_ppk($idtabmasterpptk); //ambil list ppk berdasrkan id master
                  $listppk = $modellstppk->result_array();

                  $lskegsemua  = $this->User_model->getdetlistkegiatan_semuappk($idtabmasterpptk,$this->tahunskr);
                  ...
                  for ($i=0; $i < count($arraybuln) ; $i++) {
                    ...
                    foreach ($listppk as $key => $keyppk) {
                        ...
                        $lskegppk  = $this->User_model->getdetlistkegiatan_ppk($keyppk['nipppk'],$this->tahunskr);
                        if(!$lskegppk ){
                            $datappk[]=array();
                            $datapaguppk [] =array(
                              'tahun' => 0,
                            ...
                            foreach ($lskegppk as $key => $value) {
                              $whereinppk[]=$value['id'];
                              $arrkdkegunitppk[]= $value['kdkegunit'];
                            }
                            //nilai pagu per pptk
                            $datapaguppk=$this->User_model->pagupptk( $this->tahunskr,$kdbln,$idopd,$arrkdkegunitppk);
                            // output nilai Pagu ----->>> echo json_encode($datapagu[0]['tahun']);
                            //nilai persentase
                            // $persentase =
                            $modelprsnrealppk = $this->User_model->realkeu_ppk_persen($whereinppk,$this->tahunskr,$kdbln);
                            $modelprsnlrealbmodalppk = $this->User_model->realkeu_ppk_bmodal_persen($whereinppk,$this->tahunskr,$kdbln);
                            $prsnrealppk = $modelprsnrealppk['nilai'] + $modelprsnlrealbmodalppk['nilai']; //nilai untuk mencari persen bulan sekarang

                            $modelrealsdppk = $this->User_model->realkeu_ppk_persen($whereinppk,$this->tahunskr,$kdbln,1);
                            $modelrealbmodalsdppk = $this->User_model->realkeu_ppk_bmodal_persen($whereinppk,$this->tahunskr,$kdbln,1);
                            $realsdppk =  $modelrealsdppk['nilai'] + $modelrealbmodalsdppk['nilai'];  //nilai untuk mencari persen sampai dengan bulan sekarang

                            $detppk=array();


                            foreach ($lskegppk as $key ) {
                              $kdkeg = $key['kdkegunit'];
                              $idtab = $key['id'];
                        ...
                        if($realsdppk > 0){
                          $prsnrealsdppk= number_format($realsdppk/$datapagu[0]['blnsdskr']*100,2);
                        }else{
                          $prsnrealsdppk= number_format(0,2);
                        }
                        if($prsnrealppk > 0 ){
                          $prsnrealskrppk = number_format($prsnrealppk/$datapagu[0]['blnskr']*100,2);
                        }else{
                          $prsnrealskrppk = number_format(0,2);
                        }

                        $rincian[]=array(
                          'paguppk'      => $datapaguppk,
                          'realsdppk'    => $realsdppk,
                          'prsnrealsdppk'=> $prsnrealsdppk,
                          'realbulnskrppk' => $prsnrealppk,
                          'prsnrealskrppk'  => $prsnrealskrppk,
                          'datappk'      => $datappk
                        );

                        $ppk[]=array(
                          'id'        => $keyppk['id'],
                          'idmaster'  => $keyppk['id_pptk_master'],
                          'nipppk'    => $keyppk['nipppk'],
                          'nmppk'     => $keyppk['idpnsppk'] ,
                          'nilai'     => $keyppk['nilai'],
                          'rincian'   => $rincian
                        );
                    }
            ...
            // echo json_encode($this->data);exit;
              $this->template->load('templatenew','v_eval_semua',$this->data);
...
//<--function created by ragaa
    ...
    $secretHash = "Rizky Primadona";
    // $secretHash = "yqB6gHWfslCoYKWuAcOkqWbBRGqXqMA0UdNxQyqG9Iw=";
    //yqB6gHWfslCoYKWuAcOkqWbBRGqXqMA0UdNxQyqG9Iw=
    //To encrypt
